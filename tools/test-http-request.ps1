...
try {
    ...
    while (-not $mockReady -and [DateTime]::UtcNow -lt $mockDeadline) {
        ...
        if ($mockProcess.HasExited) {
            throw 'Mock HTTP request endpoint exited before becoming ready'
        }
        try {
    ...
    if (-not $mockReady) {
        throw "Mock HTTP request endpoint did not become ready on port $upstreamPort"
    }
    ...
    if (-not $state) {
        throw 'HTTP request monitor did not publish the expected state'
    }

    Write-Output `
        "PixelStatus NX HTTP request smoke test passed: id=$($state.id), status=$($state.status), accepted=$($state.value)."
} finally {
