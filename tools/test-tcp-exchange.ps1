...
$config = Resolve-Path -LiteralPath 'examples\tcp-exchange.example.json'
$python = Get-Command python -ErrorAction Stop
$token = 'pixelstatus-tcp-exchange-integration-test'
$upstreamPort = 18080
...
try {
    ...
    while (-not $mockReady -and [DateTime]::UtcNow -lt $mockDeadline) {
        ...
        if ($mockProcess.HasExited) {
            throw 'Mock TCP exchange endpoint exited before becoming ready'
        }
        try {
    ...
    if (-not $mockReady) {
        throw "Mock TCP exchange endpoint did not become ready on port $upstreamPort"
    }
    ...
    $stateUri = "http://127.0.0.1:$ApiPort/api/v1/status/raw-http-banner"
    $state = $null
    $deadline = [DateTime]::UtcNow.AddSeconds(6)
    while (-not $state -and [DateTime]::UtcNow -lt $deadline) {
        Start-Sleep -Milliseconds 100
        if ($simulatorProcess.HasExited) {
            throw 'Simulator exited before publishing the TCP exchange state'
        }
        try {
            $candidate = Invoke-RestMethod -Uri $stateUri -Headers $headers
            if ($candidate.status -eq 'ok' `
                -and $candidate.value -match '200 OK' `
                -and $candidate.message -match '^TCP exchange completed') {
                $state = $candidate
            }
        } catch {
        }
    ...
    if (-not $state) {
        throw 'TCP exchange monitor did not publish the expected response state'
    }

    Write-Output `
        "PixelStatus NX TCP exchange smoke test passed: id=$($state.id), status=$($state.status), response contains 200 OK."
} finally {
